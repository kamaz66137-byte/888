...
7. 不确定的内容包含验证路径（禁止吹牛）
8. 快速参考中无文档堆砌
9. 读起来像操作手册，而非知识堆砌�?DeepSeek 模型实现 Agent、工具调用、技能封装等场景�?
---
...
## 何时使用本技�?
满足以下任意条件时触发：
- 需要调�?DeepSeek API（Chat / Reasoner / 工具调用�?- 正在构建 DeepSeek Agent 或封�?Skill
- 需要为 DeepSeek 配置工具（function calling�?- 需要流式输出（stream）实�?- 调试 DeepSeek API 报错�?xx / 5xx / 超时�?
## 禁止使用 / 边界

- **不涉�?* OpenAI / Anthropic 原生 SDK（仅 DeepSeek OpenAI 兼容接口�?- **不包�?* 模型训练、微调、部�?- **不生�?* API Key 或账号凭�?- 缺失以下信息时先提问：模型名称（deepseek-v4-flash / deepseek-v4-pro）、是否需要工具调用、目标语言（Python / PowerShell / Node.js�?
## 快速参�?
### 端点与模�?
| 用�?| 端点 | 推荐模型 |
|------|------|---------|
...
## 规则与约�?
- **MUST**：API Key 从环境变量读取（`DEEPSEEK_API_KEY`），禁止硬编�?- **MUST**：使�?`deepseek-v4-flash` �?`deepseek-v4-pro`，不使用已弃用模�?- **SHOULD**：工具调用后检�?`finish_reason == "tool_calls"` 再解�?- **SHOULD**：流式输出处�?`delta.content is None` 的情�?- **NEVER**：不在日�?代码中输出完�?API Key
- **NEVER**：不忽略 HTTP 429（限流）——应做指数退避重�?
## 示例

### 示例1：构建单�?Agent（Python�?- **输入**：用户问�?+ 工具列表
- **步骤**�?  1. 发送含 `tools` �?Chat 请求
  2. 检�?`finish_reason == "tool_calls"`
  3. 执行工具，将结果�?`role: tool` 追加�?messages
  4. 再次调用 API 获取最终回�?- **验收**：最终回答中包含工具执行结果，无幻觉

### 示例2：封�?DeepSeek Skill（PowerShell + SKILL.md�?- **输入**：业务场景描�?- **步骤**�?  1. �?`create-skill.sh <name> --full` 生成骨架
  2. �?`SKILL.md` 快速参考中放入 PowerShell 调用片段
  3. `validate-skill.sh <name> --strict` 验证结构
- **验收**：`EXIT:0`，中文章节标题全部通过

### 示例3：流式输�?+ 错误处理（Python�?- **输入**：长文生成请�?- **步骤**�?  1. 设置 `stream=True`
  2. �?chunk 打印 `delta.content`（跳�?`None`�?  3. 捕获 `openai.RateLimitError` 做退避重试（最�?3 次）
- **验收**：输出完整，限流时自动重试后成功

## 常见问题

- **Q: 如何选择 flash 还是 pro�?*
  - A: 简单问�?代码补全�?`flash`；复杂推�?多步 Agent �?`pro`
- **Q: Anthropic 兼容接口�?OpenAI 兼容接口有何区别�?*
  - A: 消息格式略有差异（如 `system` 字段位置）；优先�?OpenAI 兼容接口，Anthropic 兼容接口用于已有 Anthropic SDK 的项目迁�?- **Q: 工具调用返回 `finish_reason: stop` 而非 `tool_calls`�?*
  - A: 模型判断不需要调用工具，直接回答了；检查工具描述是否足够清�?
## 故障排查

- **401 Unauthorized** �?API Key 无效或未设置 `DEEPSEEK_API_KEY` 环境变量
- **429 Too Many Requests** �?触发限流，做指数退避（1s �?2s �?4s）重�?- **400 Bad Request / invalid model** �?使用了已弃用模型名，改为 `deepseek-v4-flash`
- **工具调用结果未被模型引用** �?`tool` 消息�?`tool_call_id` 与请求中不匹�?
## 参考资�?
- `references/index.md`：导航索�?- `references/api.md`：完�?API 参数参�?- `references/examples.md`：多�?Agent、RAG、批量调用扩展示�?
## 维护说明

- 来源：DeepSeek 官方 API 文档、实际工程经�?- 最后更新：2026-05-03
- 已知限制：不包含 DeepSeek 私有化部署配置；多模态接口暂未覆�?
## 质量门控

发布前最低检查项（完整版见元技�?`skills-skills`）：

1. `description` 可判定（"做什�?+ 何时�?）且包含触发关键�?2. 包含"何时使用本技�?且触发器可判�?3. 包含"禁止使用 / 边界"以减少误触发
4. 快速参�?<= 20 个模式且每个模式可直接使�?5. 包含 >= 3 个可复现示例（输�?-> 步骤 -> 验收�?6. 长篇内容�?`references/` 中，且有可导航的 `references/index.md`
7. 不确定的内容包含验证路径（禁止吹牛）
8. 快速参考中无文档堆�?9. 读起来像操作手册，而非知识堆砌
