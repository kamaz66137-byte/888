# `.github/config/.codex/` 用法说明

本目录用于在仓库内版本化管理 Codex CLI 的“全局配置基线”，便于多人同步、审阅与回滚。

你只需要把本目录里的两个文件复制到 **Codex Home**（默认 `~/.codex/`）即可生效：

- `.github/config/.codex/config.toml` → `~/.codex/config.toml`
- `.github/config/.codex/AGENTS.md` → `~/.codex/AGENTS.md`
...
```bash
mkdir -p ~/.codex
cp -f .github/config/.codex/config.toml ~/.codex/config.toml
cp -f .github/config/.codex/AGENTS.md ~/.codex/AGENTS.md
```
