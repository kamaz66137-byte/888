...
from .stats import dispatch_stats_tool
from .search_all import dispatch_search_all_tool
from .bulk import dispatch_bulk_tool
from .export_import import dispatch_export_import_tool
from .tags import dispatch_tags_tool
from .backup import dispatch_backup_tool

# ── O(1) 工具路由注册表 ─────────────────────────────────────────────────────
# 将每个工具名映射到其所属的模块分发函数，避免线性 if/elif 扫描。
# hot_events 不需要 db_path，用 lambda 统一签名。
...
_DICT_TOOLS = {"add/dict", "get/dict", "get/dict/by-name", "list/dict", "update/dict", "delete/dict"}
...
def _make_module_registry() -> dict[str, Callable[[str, dict, Path], str | None]]:
    ...
    _reg(_DICT_TOOLS, dispatch_dict_tool)
    _reg(_PROMPT_TOOLS, dispatch_prompt_tool)
    _reg(_SNIPPET_TOOLS, dispatch_snippet_tool)
    _reg(_ENV_TOOLS, dispatch_env_tool)
    _reg(_LOG_TOOLS, dispatch_log_tool)
